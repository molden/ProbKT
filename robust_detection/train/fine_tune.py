...
def hungarian_predictor_fine_tune(checkpoint_file, model_cls, data_cls, target_data_cls, target_data_path, args, logger):
   ...
   # import ipdb; ipdb.set_trace()
    ...
    wrap_model = RCNN_Predictor(len_dataloader=len(
    ...
    early_stopping_cb = EarlyStopping(
        monitor="val_acc", patience=args.early_stopping, mode="max")
    lr_monitor = LearningRateMonitor(logging_interval='step')

    trainer = pl.Trainer(gpus=args.gpus, logger=logger, callbacks=[
                         checkpoint_cb, early_stopping_cb, lr_monitor], max_epochs=args.epochs, log_every_n_steps=10, gradient_clip_val=args.gradient_clip_val)
...
def hungarian_fine_tune(run_name, model_cls, data_cls, target_data_path, args, logger):
    ...
    trainer = pl.Trainer(gpus=args.gpus, logger=logger, callbacks=[
                         checkpoint_cb, early_stopping_cb, lr_monitor], max_epochs=args.epochs, log_every_n_steps=10)
...
def re_train(checkpoint_file, model_cls, data_cls, target_data_path, target_data_cls, logger=None):
    ...
    # shifting the label index by 1 in the mnist case (0 is background in RCNN)
    elif "clevr" in target_data_path:
...
def re_train_detr(checkpoint_file, model_cls, data_cls, target_data_path, target_data_cls, logger=None):
...
if __name__ == "__main__":
    ...
    hungarian_fine_tune(run_name, model_cls, data_cls,
                        target_data_path, num_epochs_hungarian=20)
