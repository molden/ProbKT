...
def generate_dataset(dirpath: pathlib.Path,
                     ...
                     mnist_labels: np.ndarray, seed = 421):
    ...
    for image_id in tqdm.trange(num_images, desc=f"Generating dataset, saving to: {dirpath}"):
        ...
        labels = []
        bboxes = []
        num_images = np.random.randint(min_digits_per_image, max_digits_per_image+1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--min-digits-per-image", default=1, type=int
    )
    parser.add_argument(
        "--filter-digit", nargs="+", default=[8], type=int
    )
    parser.add_argument(
    ...
    filter_digit = np.array(args.filter_digit)
    result      = np.array([item not in filter_digit for item in Y_train])
    result_test = np.array([item not in filter_digit for item in Y_test])
    #result      = np.where(Y_train != args.filter_digit)
    #result_test = np.where(Y_test != args.filter_digit)
    X_train = X_train[result]
    Y_train = Y_train[result]
    Y_test = Y_test[result_test]
    X_test = X_test[result_test]
    ...
    generate_dataset(
            pathlib.Path(args.base_path, "test_ood"),
            args.num_test_images,
            args.max_digit_size,
            args.min_digit_size,
            args.imsize,
            args.max_digits_per_image+1,
            args.min_digits_per_image+1,
            X_test,
            Y_test)

    for fold in range(args.folds):
        idx_train, idx_val = train_test_split(np.arange(args.num_train_images),test_size = 0.3, random_state = 40 + fold)
        os.makedirs(os.path.join("./",args.base_path,"folds",str(fold)),exist_ok=True)
        np.save(os.path.join("./",args.base_path,"folds",str(fold),"train_idx.npy"),idx_train)
        np.save(os.path.join("./",args.base_path,"folds",str(fold),"val_idx.npy"), idx_val)
